...
public sealed class StabilityController
{
    ...
    public IReadOnlyList<StabilityCapture> Captures => _captures;
    ...
    public event Action<StabilityCapture>?   StableCaptured;
    ...
    public void OnPenData(PenReadingData d)
    {
        ...
        if (penStable && scaleStable && _lastScaleGf > 0)
        {
                ...
                (now - _lastCaptureTime).TotalMilliseconds   >= MinGapMs)
            {
                ...
                {
                    var capture = new StabilityCapture(
                        PhysicalGf:   physGf,
                        LogicalNorm:  logNorm,
    ...
    public void LoadCaptures(IEnumerable<StabilityCapture> captures)
    {
        Clear();
    ...
    /// </summary>
    public StabilityCapture? RecordManual(double physGf, double logicalNorm)
    {
