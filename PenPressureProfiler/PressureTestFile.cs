...
/// <summary>
/// Serialization model for a pressure test JSON file.
...
public class PressureTestFile
{
    ...
    /// <summary>
    /// Each element is a two-value array: [physical_gf, logical_percent].
    /// </summary>
    [JsonPropertyName("records")] public List<double[]> Records { get; set; } = [];

    /// <summary>
    /// Converts to a <see cref="PressureRecordCollection"/>, scaling logical percent → fraction.
    /// </summary>
    public PressureRecordCollection ToRecordCollection()
    {
        var collection = new PressureRecordCollection();
        foreach (var r in Records)
        {
            if (r.Length >= 2)
            {
                collection.Add(r[0], r[1] / 100.0);
            }
        }
        return collection;
    }
