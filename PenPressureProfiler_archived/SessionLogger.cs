...
/// in <see cref="LogDirectory"/>. Create once, call Start/Stop as needed.
/// </summary>
public sealed class SessionLogger : IDisposable
{
    ...
    public void LogScaleReading(string strForce)
    {
        if (!IsLogging || _scaleWriter is null) return;
        _scaleWriter.WriteLine($"{Ts()},{strForce}");
    }
