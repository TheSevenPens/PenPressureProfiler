...
- **Accumulator** mode: set the force **Range** (default 0–10 gf) and **Bucket** size (default 0.5 gf), click **Start**, and sweep force up and down across the range a few times. The **BUCKETS** table fills in per-bucket **0% / >0% / %ON** counts (plus the `< min` / `≥ max` out-of-range rows), the centre chart's activation-% markers (sized by sample count) settle around the dotted 50% reference line, and **Samples / Est. IAF** update. Once a bucket has ≥ 50 samples its row tints (≤20% on → very light blue, ≥80% on → very light purple); the active cell flashes orange. Change the **Bucket** size mid-run and confirm the data is preserved (all widths accumulate at once); change the **Range** and confirm that resets the data. Toggle **Apply scale-lag comp (245 ms)** and confirm **Clear** resets the counts and chart.
