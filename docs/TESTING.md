...
- **Accumulator** mode: pick the **Measure** target (**IAF** default, or **Max pressure**), set the force **Range** and **Bucket** size (defaults: IAF 0–10 gf / 0.5 gf, Max 0–500 gf / 25 gf — edit by typing, arrows, or wheel with **Shift = ×5**; the step is per-target), click **Start**, and sweep force up and down across the range a few times. The **BUCKETS** table fills in per-bucket **under / at-or-over / %ON** counts — headers read `0% / >0%` for IAF, `<max / max` for Max — plus the `< min` / `≥ max` out-of-range rows; the centre chart's markers (sized by sample count) settle around the dotted 50% line, a **live vertical force line** tracks the scale, and **Samples / Est. IAF (or Est. Max)** update. Once a bucket has ≥ 50 samples its row tints (≤20% → very light blue, ≥80% → very light purple); the active cell flashes orange. Switch **Measure** and confirm each target keeps its own range/buckets/data. Change the **Bucket** size mid-run and confirm the data is preserved; change the **Range** and confirm that resets the active target's data. Toggle **Apply scale-lag comp (245 ms)** and confirm **Clear** resets the counts and chart.
